...
// NOTE: Debug deployment script removed - subscription policy requires managed identity for deployment scripts
// Key Vault deployment parameters: deployKeyVault=${deployKeyVault}, adminObjectId provided: ${!empty(adminObjectId)}
...
resource keyVaultResource 'Microsoft.KeyVault/vaults@2022-07-01' = if (deployKeyVault) {
  name: 'kv-az700-${uniqueString(resourceGroup().id)}'
  location: location
  tags: keyVaultTags
  properties: {
    sku: {
...
module storageKeyAccess 'modules/services/keyvaultAccess.bicep' = if (deployKeyVault && enableCmkForStorage) {
